...
class single_block{
    ...
    float xPos, yPos;
    void make(){
        if(arena.filled[((int)xPos)+1][((int)yPos)-1] == 0){
            moveCursor((xPos)+1, (yPos)-1);
            cout << "_";
        }
        //if((xPos < arenaWidth) && (xPos > 0) && (yPos+1 < arenaHeight) && (arena.filled[(int)xPos][(int)yPos] == 0)){
        if((xPos < arenaWidth) && (xPos > 0) && (arena.filled[(int)xPos][(int)yPos] == 0)){
            moveCursor(xPos, yPos);
            cout << "|";
        }
        //if((xPos+1 < arenaWidth) && (xPos+1) > 0 && (yPos+1 < arenaHeight) && (arena.filled[(int)xPos+1][(int)yPos] == 0)){
        if((xPos+1 < arenaWidth) && (xPos+1) > 0 && (arena.filled[(int)xPos+1][(int)yPos] == 0)){
            moveCursor(xPos+1, yPos);
            cout << "_";
        }
        //if((xPos+2 < arenaWidth) && (xPos > 0) && (yPos+1 < arenaHeight) && (arena.filled[(int)xPos+2][(int)yPos] == 0)){
        if((xPos+2 < arenaWidth) && (xPos > 0) && (arena.filled[(int)xPos+2][(int)yPos] == 0)){
            moveCursor(xPos+2, yPos);
            cout << "|";
        }
    }
    void destroy(int xShift, int yShift){
        if(arena.filled[((int)xPos+xShift)][((int)yPos+yShift)-1] == 0){
            moveCursor((xPos+xShift)+1, (yPos+yShift)-1);
            cout << " ";
        }
        if((xPos+xShift < arenaWidth) && (xPos+xShift > 0) && (yPos+yShift+1 < arenaHeight) && (arena.filled[(int)xPos+xShift-2][(int)yPos+yShift] == 0)){
            moveCursor(xPos+xShift, yPos+yShift);
            cout << " ";
        }
        if((xPos+xShift+1 < arenaWidth) && (xPos+xShift+1) > 0 && (yPos+yShift+1 < arenaHeight) && (arena.filled[(int)xPos+xShift][(int)yPos+yShift+1] == 0)){
            moveCursor(xPos+xShift+1, yPos+yShift);
            cout << " ";
        }
        if(xPos+xShift+2 < arenaWidth && xPos+xShift > 0 && yPos+yShift+1 < arenaHeight && (arena.filled[(int)xPos+xShift+2][(int)yPos+yShift] == 0)){
            moveCursor(xPos+xShift+2, yPos+yShift);
            cout << " ";
        }
    }
};
...
class shape{
    ...
    int move(){ //wrappper for movement functions
        ...
        if((clock() - dropTimer) / (double)CLOCKS_PER_SEC > .5){ //every half second, drop piece one block
            erase(0,0); //drop() hasn't moved the piece yet, so erase the original piece
        ...
        if(shiftAmt != 0 || dropAmt != 0){ //if the block was shifted or dropped
            erase(shiftAmt*-2, dropAmt*-1); //erase the old block
        }
        draw();
        return 1;
    }
    ...
    int canDrop(){ //function to check if the piece can drop
        for(int i = 0; i < 4; i++){ //loop from 0 to 3
            ...
            if(arena.filled[x][y+1] != 0){ //if the spot the block is going to move to is filled
                return 0; //return that it cannot drop
            }
        }
    ...
    int canRotate(){
        for(int rotation = 0; rotation < 4; rotation++){ //loop from 0 to 3
            for(int i = 0; i < 4; i++){
                if(blocks[rotation][i].xPos == 0.1){
                    return 0;
                }
                if(blocks[rotation][i].xPos <= 0){ //if any block in the new rotation would move it out of the arena
                    return 0; //return that nothing was changed
                }
                if(blocks[rotation][i].xPos+2 >= arenaWidth){ //same as above if statement
                    return 0; //return that nothing was changed
                }
                if(blocks[rotation][i].yPos+1 >= arenaHeight){ //same as above if statement
                    return 0; //return that nothing was changed
                }
                int x = blocks[rotation][i].xPos;
                int y = blocks[rotation][i].yPos;
                if(arena.filled[x][y] == 1){ //if the spot the block is going to move to is filled
                    return 0; //return that it cannot drop
                }
            }
        }
    ...
    void drop(){ //function to drop the piece
        for(int i = 0; i < 4; i++){ //loop from 0 to 3
            for(int it = 0; it < 4; it++){ //loop from 0 to 3
                if(blocks[i][it].yPos != 0.1){ //if the block's y position is changed (initialized to -1)
                    arena.filled[(int)blocks[i][it].xPos][(int)blocks[i][it].yPos] = 0;
                    blocks[i][it].yPos++; //drop it one space. do so for each loop, for every piece in the block
                    arena.filled[(int)blocks[i][it].xPos][(int)blocks[i][it].yPos] = 1;
                }
            }
        }
    }
    int gameDrop(){ //game's drop function. no user input
        drop(); //just drop the piece 1 space
        ...
        if((GetKeyState(VK_LEFT) & 0x80) != 0){ //if the left key is pressed
            ...
            for(int i = 0; i < 4; i++){ //loop from 0 to 3
                if(blocks[rotation][i].xPos+3 >= arenaWidth && blocks[rotation][i].xPos != 0.1){ //if the new position would be outside of the arena
                    //note: it is xPos+3 because the x position is this part of the piece -->|_|, so it needs to check past the other 2 pieces
                    return 0; //exit the function, return that it did not move any blocks
                }
                int x = blocks[rotation][i].xPos;
                int y = blocks[rotation][i].yPos;
    ...
    void erase(int xShift, int yShift){ //function to call destroy function for individual blocks
        blocks[rotation][0].destroy(xShift, yShift); //destroy first block
        blocks[rotation][1].destroy(xShift, yShift); //destroy second
        blocks[rotation][2].destroy(xShift, yShift); //destroy third
        blocks[rotation][3].destroy(xShift, yShift); //destroy fourth
    }
